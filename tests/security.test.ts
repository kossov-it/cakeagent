...
const MUST_ALLOW: Array<[string, string]> = [
  ...
  ['nft add rule', 'nft add rule inet filter input tcp dport 80 accept'],
  ['nft delete rule', 'nft delete rule inet filter input handle 7'],
  ['iptables add rule', 'iptables -A INPUT -p tcp --dport 443 -j ACCEPT'],
  ['iptables delete rule', 'iptables -D INPUT -p tcp --dport 443 -j ACCEPT'],
